package com.android.lv.imageswitcher.view.imageindicator;
...
public class AutoPlayManager {
	 ...
	 */
	private WeakReference<ImageIndicatorView> mImageIndicatorView = null;

	public AutoPlayManager(ImageIndicatorView imageIndicatorView) {
		this.mImageIndicatorView = new WeakReference<ImageIndicatorView>(imageIndicatorView);
	...
	protected void handleMessage(Message msg) {
		if (broadcastEnable && mImageIndicatorView.get()!=null) {
			...
			if (direction == RIGHT) {// roll right
				if (mImageIndicatorView.get().getCurrentIndex() < mImageIndicatorView.get()
						.getTotalCount()) {
					if (mImageIndicatorView.get().getCurrentIndex() == mImageIndicatorView.get()
							.getTotalCount() - 1) {
						timesCount++;// 循环次数次数加1
						...
						mImageIndicatorView.get()
								.getViewPager()
								.setCurrentItem(
										mImageIndicatorView.get().getCurrentIndex() + 1,
										true);
					}
				}
			} else {// roll left
				if (mImageIndicatorView.get().getCurrentIndex() >= 0) {
					if (mImageIndicatorView.get().getCurrentIndex() == 0) {
						direction = RIGHT;
					} else {
						mImageIndicatorView.get()
