...
public class UpdateEveryDayImageActivity extends BaseActivity {

    private static final String TAG = "UpdateEveryDayImageActivity" ;
    ...
    private DelayImageItem imageItem = null;
    ...
    public void rend() {
        ...
        date_picker.showProbablyTime(false);
        date_picker.showSpecificTime(true); // 显示时和分
        date_picker.setIsLoop(true); // 允许循环滚动

        if(imageId > 0){
            imageItem = m_application.getDelayImage(imageId);
        }

        if(imageItem != null){
            String now = DateUtil.formatDateMM(imageItem.getImageDate());
        ...
        public void onClick(View v) {
            switch (v.getId()){
                ...
                case R.id.rl_add_image:
                    ...
                    date_picker.show("1970-01-01 " + tv_image_date.getText().toString());
                    break;

                case R.id.bt_add_deter:
                    if(imagePath == null || imagePath.equals(""))
                    {
                        Toast.makeText(UpdateEveryDayImageActivity.this, "请选择图片", Toast.LENGTH_SHORT).show();
                        return;
                    }

                    if(tv_image_date.getText().toString() != null && !"".equals(tv_image_date.getText().toString())){
                        imageDate = getStamp("1970-01-01 " + tv_image_date.getText().toString());
                    }

                    Log.i(TAG, "[imagePath]" + imagePath);
                    Log.i(TAG, "[imageDate]" + String.valueOf(imageDate));

                    DelayImageItem imageItem = new DelayImageItem();
                    imageItem.setImagePath(imagePath);
                    imageItem.setImageDate(imageDate);

                    m_application.updateDelayImage(imageId, imageItem);

                    Log.i(TAG, "[Size]" + m_application.getImages().size());
                    Toast.makeText(UpdateEveryDayImageActivity.this, "修改成功", Toast.LENGTH_SHORT).show();
                    finish();
    ...
    private void showImage(String path){
        ...
        imagePath = path;
        Glide.with(UpdateEveryDayImageActivity.this)
