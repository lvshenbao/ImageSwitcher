...
public class AddEveryDayImageActivity extends BaseActivity {

    private static final String TAG = "AddTodayImageActivity" ;
    ...
    public void rend() {
        ...
        date_picker.showProbablyTime(false); // 不显示年月日
        date_picker.showSpecificTime(true); // 显示时和分
        date_picker.setIsLoop(true); // 允许循环滚动

        String now = getDateFormat(System.currentTimeMillis());
        ...
        public void onClick(View v) {
            switch (v.getId()){
                ...
                case R.id.bt_add_deter:
                    ...
                    {
                        Toast.makeText(AddEveryDayImageActivity.this, "请选择图片", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    ...
                    m_application.saveDelayImage(imageItem);

                    Log.i(TAG, "[Size]" + m_application.getImages().size());
                    Toast.makeText(AddEveryDayImageActivity.this, "添加成功", Toast.LENGTH_SHORT).show();
                    finish();
                    break;
            }
        }
    ...
    private void showImage(String path){
        ...
        imagePath = path;
        Glide.with(AddEveryDayImageActivity.this)
